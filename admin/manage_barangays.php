...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_barangay'])) {
    ...
    if (!$name) {
        ...
        if ($chk && $chk->num_rows > 0) {
            ...
            $conn->query("INSERT INTO barangays (name) VALUES ('$name')");
...
");
if (!$barangays) die("Query failed: " . $conn->error);

$all = [];
while ($r = $barangays->fetch_assoc()) $all[] = $r;
?>
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-4">
                    <form method="POST">
                        <div class="fc-form-section">
                            ...
                            <div class="mb-3">
                                <label class="fc-form-label">Barangay Name <span style="color:var(--fc-primary)">*</span></label>
                                <input type="text" name="barangay_name" class="fc-form-control"
                                       placeholder="e.g. Agdao" required>
                            </div>
                            <button type="submit" name="add_barangay" class="fc-btn fc-btn-primary" style="width:100%;justify-content:center;">
                                <i class="bi bi-plus-circle-fill"></i> Add Barangay
                            </button>
                        </div>
                    </form>
                    ...
                    <div class="fc-form-section" style="margin-top:20px;">
                        ...
                        if ($top && $top->num_rows > 0):
                            $max_cnt = null;
                            $top_arr = [];
                            while ($t = $top->fetch_assoc()) { $top_arr[] = $t; if ($max_cnt === null) $max_cnt = $t['cnt']; }
                        ?>
                        ...
                        <div style="margin-bottom:10px;">
                            <div style="display:flex;justify-content:space-between;font-size:12.5px;margin-bottom:4px;">
                                <span style="font-weight:600;color:var(--fc-dark);"><?= htmlspecialchars($t['name']) ?></span>
                                <span style="color:var(--fc-muted);"><?= $t['cnt'] ?> report<?= $t['cnt']!=1?'s':'' ?></span>
                            </div>
                            <div style="background:#f1f5f9;border-radius:100px;height:6px;overflow:hidden;">
                                <div style="width:<?= round(($t['cnt']/$max_cnt)*100) ?>%;background:var(--fc-primary);height:100%;border-radius:100px;"></div>
                            </div>
                        </div>
                        <?php endforeach; ?>
                        <?php else: ?>
                ...
                <div class="col-lg-8">
                    ...
                    <form method="GET" style="margin-bottom:14px;display:flex;gap:10px;">
                        ...
                        <?php if ($search): ?>
                        <a href="manage_barangays.php" class="fc-btn" style="padding:9px 14px;font-size:13px;background:#fff;border:1.5px solid var(--fc-border);color:var(--fc-text);">
                            <i class="bi bi-x"></i>
                        </a>
                        <?php endif; ?>
                    </form>
                    ...
                    <div class="fc-card">
                        ...
                        <?php if (empty($all)): ?>
                        <div class="fc-empty"><i class="bi bi-geo-alt"></i><h6>No barangays found</h6></div>
                        <?php else: ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div style="display:flex;align-items:center;gap:8px;">
                                                <i class="bi bi-geo-alt-fill" style="color:var(--fc-primary);font-size:13px;"></i>
                                        ...
                                        <td>
                                            <div style="display:flex;gap:6px;">
                                                ...
                                                <a href="?delete=<?= $b['id'] ?><?= $search?"&search=$search":'' ?>"
                                                   ...
                                                   onclick="return confirm('Delete barangay \"<?= htmlspecialchars(addslashes($b['name'])) ?>\"?')">
                                                    <i class="bi bi-trash-fill"></i>
                                                </a>
                                                <?php else: ?>
                                                <span class="fc-icon-btn" style="opacity:.3;cursor:default;" title="In use — cannot delete">
