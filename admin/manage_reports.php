...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_team'])) {
    ...
    $type = $typeNormalize[strtolower($type)] ?? $type;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_team'])) {
    ...
    // Normalize type to ensure DB consistency
    $typeNormalize = [
        'fire' => 'Fire',
        'medical' => 'Medical Emergency',
        'crime' => 'Crime / Security',
...
<script>
    ...
    function viewTeamDetails(id, name, type, lead, memberCount, status) {
        ...
        // Map DB value to readable label
        const typeMap = {
            'fire': 'Fire',
            'medical': 'Medical Emergency',
    ...
    function openEditModal(id, name, type, lead, imagePath) {
        ...
        // Set team type select — normalize DB value to match option values
        const sel = document.getElementById('editTeamType');
        // Map lowercase/shorthand DB values to full option values
        const typeMap = {
            'fire': 'Fire',
            'medical': 'Medical Emergency',
            'medical emergency': 'Medical Emergency',
        ...
        sel.value = normalizedType;
        // If still no match, try exact
        if (!sel.value) sel.value = type;
